...
public final class GenerateTool {
    ...
    public static List<ConfigTreeDTO> convertToConfigTreeDTO(String json) {
        ...
        for (Object o : list) {
            ConfigTreeDTO data = JSON.parseObject(JSON.toJSONString(o), ConfigTreeDTO.class);
            result.add(data);
        }
        return result;
    }
    ...
    public static List<GenTableColumnDO> filterCustom(List<GenTableColumnDO> columnList, String columns) {
        if (!com.xht.cloud.framework.core.support.StringUtils.hasText(columns)) {
            return columnList;
        }
        Set<String> columnNameList = StrUtil.split(columns, ",").stream().map(String::toLowerCase).collect(Collectors.toSet());
        return columnList.stream().filter(item -> columnNameList.contains(item.getColumnDbType().toLowerCase())).collect(Collectors.toList());
    }
