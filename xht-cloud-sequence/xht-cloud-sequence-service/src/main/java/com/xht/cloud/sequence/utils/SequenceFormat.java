package com.xht.cloud.sequence.utils;
...
public class SequenceFormat {
    ...
    public static String format(SysSequenceDO sysSequenceDO) {
        ...
        Assert.isFalse(currentValue > maxValue && Objects.equals(SeqLoopEnum.NO_LOOP, seqLoop), "当前值大于该序列最大值");
        if (currentValue > maxValue) {
            currentValue = minValue;
        }
        currentValue = currentValue + stepValue;
        sysSequenceDO.setCurrentValue(currentValue);
        Assert.isFalse(currentValue < minValue, "当前值小于该序列最小值");
