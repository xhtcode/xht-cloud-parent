...
public class ClientEndpointAuthenticationFailureHandler implements AuthenticationFailureHandler {

    protected final Converter<OAuth2Error, Map<String, String>> errorParametersConverter;

    public ClientEndpointAuthenticationFailureHandler() {
        this.errorParametersConverter = new OAuth2ErrorParametersConverter();
        log.info("client 端点 失败时调用");
    }
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        WebUtil.renderString(response, HttpStatus.BAD_REQUEST, R.restResult(401, "client_id或client_secret错误!", errorParameters));
        log.error("client endpoint 异常 error={}", errorParameters, exception);
    }
